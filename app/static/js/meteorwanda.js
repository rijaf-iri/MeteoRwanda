var mapCenterLON = 29.85;
var mapCenterLAT = -1.95;
var radarLON = 30.1128;
var radarLAT = -2.158042;
var airportLON = 30.1455;
var airportLAT = -1.9711;
...
$.getJSON(Flask.url_for("static", {
    "filename": "json/AWS_10MinVarObj.json"
}), (json) => {
    AWS_10MinVarObj = json.variables;
});

//
var AWS_AggrTsObj;
$.getJSON(Flask.url_for("static", {
    "filename": "json/AWS_AggrTsObj.json"
}), (json) => {
    AWS_AggrTsObj = json.variables;
});

//
var AWS_AggrSpObj;
$.getJSON(Flask.url_for("static", {
    "filename": "json/AWS_AggrSpObj.json"
}), (json) => {
    AWS_AggrSpObj = json.variables;
});

// 
var AWS_DataVarObj;
$.getJSON(Flask.url_for("static", {
    "filename": "json/AWS_DataVarObj.json"
}), (json) => {
    AWS_DataVarObj = json.variables;
});

/////////////////

var RADAR_DATA = new Object();
RADAR_DATA.status = "no-data";
RADAR_DATA.msg = "not-loaded";

// 
var radarPolar_ops_sur;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarPolar_ops_sur.json"
}), (json) => {
    radarPolar_ops_sur = json.fields;
});

var radarPolar_ws_sur;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarPolar_ws_sur.json"
}), (json) => {
    radarPolar_ws_sur = json.fields;
});

var radarPolar_rate_titan;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarPolar_rate_titan.json"
}), (json) => {
    radarPolar_rate_titan = json.fields;
});

var radarPolar_rate_user;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarPolar_rate_user.json"
}), (json) => {
    radarPolar_rate_user = json.fields;
    for (var i = 0; i < radarPolar_rate_user.length; i++) {
        updateNestedObject(radarPolar_rate_user[i], ['cmdflag'], true);
        updateNestedObject(radarPolar_rate_user[i], ['cmdmask'], 'y');
    }
});

var radarCart_ops;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarCart_ops.json"
}), (json) => {
    radarCart_ops = json.fields;
});

var radarCart_echo_tops;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarCart_echo_tops.json"
}), (json) => {
    radarCart_echo_tops = json.fields;
});

var radarCart_ctrec;
$.getJSON(Flask.url_for("static", {
    "filename": "json/radarCart_ctrec.json"
}), (json) => {
    radarCart_ctrec = json.fields;
});
...
function getVarNameColorKey(pars) {
    ...
    if (["TMIN", "TMAX", "TAVG"].includes(pars)) {
        return 'TT';
    }
    if (["RHMIN", "RHMAX", "RHAVG"].includes(pars)) {
        return 'RH';
    }
    if (["RADMIN", "RADMAX", "RADAVG"].includes(pars)) {
        return 'RG';
    }
...
function createLeafletTileLayer(container, aws_tile = true) {
    if (mymapBE == undefined) {
        ...
        // 
        var meteo = ' | <a href="https://www.meteorwanda.gov.rw">MeteoRwanda</a>';
        if (aws_tile) {
...
function awsSpatialbindPopup(don, date, labelObj, labelT) {
    ...
    for (i = 0; i < labelObj.length; i++) {
        ...
        var col1 = $('<td>').text(labelObj[i].name);
        row.append(col1);
        var val = don[labelObj[i].var];
