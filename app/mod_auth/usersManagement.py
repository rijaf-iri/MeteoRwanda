...
def createUser_page():
    try:
        ...
        if request.method == "POST" and form.validate():
            ...
            sqlCmd = "SELECT * FROM mrusers WHERE username = (%s)"
            ...
            if int(rep) > 0:
                ...
                sqlCmd = "INSERT INTO mrusers (username, password, email, fullname, institution, userlevel) VALUES (%s, %s, %s, %s, %s, %s)"
...
def getListOfUsers():
    cursor, conn = connection()
    cursor.execute("SELECT * FROM mrusers")
...
def removeUser():
    ...
    cursor, conn = connection()
    sqlCmd = "SELECT username, userlevel, email, fullname, institution FROM mrusers WHERE username = (%s)"
    rep = cursor.execute(sqlCmd, thwart(username))

    if int(rep) > 0:
        json_data = convert2json(cursor)
        sqlCmd = "DELETE FROM mrusers WHERE username = (%s)"
